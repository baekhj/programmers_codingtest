...
public class 탑 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("int[] heights = {6, 9, 5, 7, 4}  결과");
        for(int a:result) System.out.println("\t "+ a);
        System.out.println("");

        heights = new int[]{3, 9, 9, 3, 5, 7, 2};
        result = solution(heights);
        System.out.println("int[] heights = {3, 9, 9, 3, 5, 7, 2}  결과");
        for(int a:result) System.out.println("\t "+ a);
        System.out.println("");

        heights = new int[]{1, 5, 3, 6, 7, 6, 5};
        result = solution(heights);
        System.out.println("int[] heights = {1, 5, 3, 6, 7, 6, 5}  결과");
        for(int a:result) System.out.println("\t "+ a);
        System.out.println("");
    ...
    */
    ...
    public static int[] solution(int[] heights){
        int[] answer = new int[heights.length]; //결과값 배열
        int defTop = 0;
        for(int x=heights.length-1; x>=0; x--){
            defTop = 0;
            for(int y=x-1; y>=0; y--) {
                if(heights[x]<heights[y]) {
                    defTop = y+1;   //탑 순번이 1부터 시작이니까 +1
                    break;
                }
            }
            answer[x] = defTop;
